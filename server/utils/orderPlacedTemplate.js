const orderPlacedTemplate = ({ name, paymentType, totalAmount, itemCount, orderIds, orderLink }) => {
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 640px; margin: 0 auto; padding: 20px; background-color: #f9fafb;">
      <div style="background-color: #ffffff; border-radius: 12px; padding: 28px; border: 1px solid #e5e7eb;">
        <h2 style="color: #16a34a; margin-bottom: 16px;">Order Placed Successfully</h2>
        ...
        <div style="background-color: #f3f4f6; border-radius: 10px; padding: 16px; margin: 18px 0;">
          <p style="margin: 0 0 8px; color: #6b7280; font-size: 13px; text-transform: uppercase; letter-spacing: 0.6px;">Order summary</p>
          <p style="margin: 6px 0; font-size: 15px; color: #111827;"><strong>Payment:</strong> ${paymentType}</p>
          <p style="margin: 6px 0; font-size: 15px; color: #111827;"><strong>Items:</strong> ${itemCount}</p>
          <p style="margin: 6px 0; font-size: 15px; color: #111827;"><strong>Total:</strong> ₹${Number(totalAmount).toFixed(2)}</p>
          <p style="margin: 6px 0; font-size: 15px; color: #111827;"><strong>Order IDs:</strong> ${orderNumberText}</p>
        </div>

        <p style="font-size: 14px; color: #6b7280; line-height: 1.6; margin: 0;">
          Thank you for shopping with DailyBasket. You can track your order from your account.
        </p>
